...
trait DependentRepo extends DependentTable {
  ...
  def getAllDependents: Future[List[Dependent]] = db.run{
    DependentQuery.to[List].result
  }

  def customQuery: Future[List[Dependent]] = db.run{
    DependentQuery.filter(_.referenceEmpId === 11).to[List].result
  }



}
object ForDependentRepo extends DependentRepo with MySqlDBComponent
